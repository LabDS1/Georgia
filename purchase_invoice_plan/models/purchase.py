...
class PurchaseInvoicePlan(models.Model):
    ...
    def _compute_new_invoice_quantity(self, invoice_move):
        ...
        if self.last:  # For last install, let the system do the calc.
            ...
            invoices = self.env['account.move'].search([('line_ids.purchase_line_id.order_id','=',self.purchase_id.id),('id','!=',move.id)])
            for line in move.invoice_line_ids:
                order_line = fields.first(line.purchase_line_id)
                line.write({"quantity":order_line.product_uom_qty})
            for invoice in invoices:
                for record_line in invoice.line_ids:
                    if record_line.product_id:
                        move.write({
                            'invoice_line_ids' :[(0,0,{'product_id':record_line.product_id.id,'quantity':-abs(record_line.quantity),'price_unit':record_line.price_unit,'tax_ids': [(6, 0, [])],'sequence':record_line.sequence,'account_id':record_line.account_id,'purchase_line_id':record_line.purchase_line_id.id})]
                        })
            return
