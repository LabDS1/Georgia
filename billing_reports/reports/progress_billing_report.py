...
class ProgressBillingReportXlsx(models.AbstractModel):
    ...
    def _generate_report_data(self, start_date, end_date):
        ...
        for so in orders:
            ...
            bills = bills.filtered(lambda x: x.state == 'posted')
            ...
            if check_invoices or check_bills:
                ...
                inv_total = round(
                    sum(
                        inv.amount_untaxed if inv.amount_total_signed >= 0 else -inv.amount_untaxed
                        for inv in filtered_invoices
                    ),
                    2
                ) if filtered_invoices else 0
    ...
    def _write_headers(self, sheet, format_header):
        ...
        sheet.merge_range('L5:L6', 'VENDOR BILL TOTAL', format_header)
